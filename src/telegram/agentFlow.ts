...
export async function runAgentWithGate(
  ...
  if ("error" in gate) {
    await ctx.reply(`Gate warning: ${gate.error}\nStarting agent without AI gate.`);
    await enqueueAndReply(ctx, draftPrompt, telegramUserId, chatId, repository, ref, clarifications);
    return;
  }
  ...
  if (depth >= config.deepseek.maxClarifyRounds) {
    await ctx.reply(
      `Reached max clarification rounds (${config.deepseek.maxClarifyRounds}). Starting agent with gathered context.`
    );
    await enqueueAndReply(ctx, draftPrompt, telegramUserId, chatId, repository, ref, clarifications);
    return;
  }
  ...
  if (!gate.question || gate.quick_replies.length === 0) {
    await ctx.reply(gate.question ?? "Add more detail, then try /agent again.");
    return;
  }
  ...
  const rows = [
    ...
    [Markup.button.callback("Cancel", `ax:${id}`)],
  ];

  const text = `${gate.question}\n\n(Reply with a button or cancel.)`;
...
export async function handleAgentClarifyCallback(ctx: Context, pendingId: string, optionIndex: number): Promise<void> {
  ...
  if (!row || row.telegram_user_id !== ctx.from?.id || row.action !== "agent_clarify") {
    await ctx.answerCbQuery("Invalid or expired.");
    try {
      await ctx.editMessageText("This prompt expired. Run /agent again.");
    } catch {
      /* ignore */
    }
    return;
